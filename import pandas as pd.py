...
def parse_dates_vectorized(series):
    ...
    for fmt in ['%d.%m.%Y', '%d.%m.%Y %H:%M:%S', '%d/%m/%Y', '%d-%m-%Y', '%Y-%m-%d']:
        mask = result.isna()
        if mask.any():
            temp = pd.to_datetime(series[mask], format=fmt, errors='coerce')
            result.loc[mask] = temp
    # For remaining, try dayfirst
    mask = result.isna()
    if mask.any():
        temp = pd.to_datetime(series[mask], dayfirst=True, errors='coerce')
        result.loc[mask] = temp
    return result
...
# Перетворення числових стовпців (крім дат та вже перетвореної "Система")
numeric_columns = [col for col in df.columns if col not in date_columns and col != "Система"]
